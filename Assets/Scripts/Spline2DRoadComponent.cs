...
public class Spline2DRoadComponent : MonoBehaviour
{
    private Spline2DRoad spline;
    ...
    private void InitSpline()
    {
        if(spline == null)
        {
            spline = new Spline2DRoad(points, closed, curvature, lengthSamplesPerSegment, width);
        }
    }
    ...
    {
        InitSpline();
        return spline.GetSidePoints(index);
    }

    public List<Tuple<Vector2, Vector2>> GetAllSidePoints()
    {
        InitSpline();
        return spline.GetAllSidePoints();
    }
    ...
    /// Get a single point in world space
    public Vector3[] GetSidePointsWorldSpace(int index)
    {
        Vector2[] p = GetSidePoints(index);
        Vector3[] points = new Vector3[p.Length];

        points[0] = p[0];
        points[1] = p[1];
        if(displayXZ)
        {
            points[0] = FlipXYtoXZ(points[0]);
            points[1] = FlipXYtoXZ(points[1]);
        }
    ...
    {
        InitSpline();
        return spline.InterpolateSides(t);
    }

    /// Interpolate a position on the entire curve in world space. Note that if the control
    /// points are not evenly spaced, this may result in varying speeds.
    public Vector3[] InterpolateSidesWorldSpace(float t)
    {
        Vector2[] p = InterpolateSides(t);
        Vector3[] points = new Vector3[p.Length];
        points[0] = p[0];
        points[1] = p[1];
        if(displayXZ)
    ...
    private void DrawCurveGizmo()
    {
        ...
        Vector3[] sidesLast = GetSidePointsWorldSpace(0);
        Vector3 plast = GetPointWorldSpace(0);
        for(float t = sampleWidth; t <= 1.0f; t += sampleWidth)
        {
            Vector3 p = InterpolateWorldSpace(t);
            Vector3[] sides = InterpolateSidesWorldSpace(t);

            Gizmos.DrawLine(sidesLast[0], sides[0]);
            Gizmos.DrawLine(plast, p);
            Gizmos.DrawLine(sidesLast[1], sides[1]);
            plast = p;
            sidesLast = sides;
        }
    }
