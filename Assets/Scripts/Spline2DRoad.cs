...
public class Spline2DRoad
{
    ...
    public Spline2DRoad(List<Vector2> intersectionPoints, bool isClosed = false, float curve = 0.5f,
        int samplesPerSegment = 5, float width = 1.0f)
    {
    ...
    private Vector2 GetCrossProduct(Vector2 p)
    {
        Vector3 crossProduct = Vector3.Cross(p, Vector3.forward).normalized;
        return new Vector2(crossProduct.x, crossProduct.y);
    }
    ...
    public Vector2[] GetSidePoints(int index)
    {
        Assert.IsTrue(index < points.Count, "Spline2D: point index out of range");
        ...
        if(index == 0)
        {
            crossProduct = Vector3.Cross(points[index + 1] - points[index], Vector3.forward).normalized;
            cross = new Vector2(crossProduct.x, crossProduct.y);
        }
        else if(index == points.Count - 1)
        {
            crossProduct = Vector3.Cross(points[index] - points[index - 1], Vector3.forward).normalized;
            cross = new Vector2(crossProduct.x, crossProduct.y);
        }
        else
        {
            crossProduct = Vector3.Cross(((points[index + 1] - points[index]) + (points[index] - points[index - 1])) / 2, Vector3.forward).normalized;
            cross = new Vector2(crossProduct.x, crossProduct.y);
        }
        ...
        sides[0] = points[index] + cross * roadWidth;
        sides[1] = points[index] - cross * roadWidth;

        return sides;
    }

    public List<Tuple<Vector2, Vector2>> GetAllSidePoints()
    {
        List<Tuple<Vector2, Vector2>> sidePoints = new List<Tuple<Vector2, Vector2>>();
        for(int i = 0; i < points.Count; i++)
        {
            Vector2[] sides = GetSidePoints(i);
            sidePoints.Add(new Tuple<Vector2, Vector2>(sides[0], sides[1]));
        }
    ...
    public Vector2[] InterpolateSides(float t)
    {
        Recalculate(false);
        ...
        return InterpolateSides(segIdx, tSeg);
    }
    ...
    public Vector2[] InterpolateSides(int fromIndex, float t)
    {
        Recalculate(false);
        ...
        if(toIndex >= points.Count)
        {
            ...
            {
                return GetSidePoints(points.Count - 1);
            }
        }
        ...
        if(Mathf.Approximately(t, 0.0f))
        {
            point = points[fromIndex];
            // Clamp to end
            sides[0] = point + new Vector2(-tangents[fromIndex].y, tangents[fromIndex].x);
            sides[1] = point - new Vector2(-tangents[fromIndex].y, tangents[fromIndex].x);

            return GetSidePoints(fromIndex);
        }
        else if(Mathf.Approximately(t, 1.0f))
        {
            point = points[toIndex];
            // Clamp to end
            crossProduct = Vector3.Cross(new Vector3(point.x, point.y), Vector3.forward).normalized;
            cross = new Vector2(crossProduct.x, crossProduct.y);
            sides[0] = point + new Vector2(-tangents[toIndex].y, tangents[toIndex].x);
            sides[1] = point - new Vector2(-tangents[toIndex].y, tangents[toIndex].x);

            return GetSidePoints(toIndex);
        }
        ...
        point = points[fromIndex];
        Vector2 pointTo = points[toIndex];

        // Clamp to end
        crossProduct = Vector3.Cross(new Vector3(point.x, point.y), Vector3.forward).normalized;
        cross = new Vector2(crossProduct.x, crossProduct.y);

        Vector3 crossProductToIndex = Vector3.Cross(new Vector3(pointTo.x, pointTo.y), Vector3.forward).normalized;

        Vector2 crossToIndex = new Vector2(crossProductToIndex.x, crossProductToIndex.y);

        //sides[0] = h1 * (points[fromIndex] + new Vector2(-tangents[fromIndex].y, tangents[fromIndex].x)) +
        //h2 * (points[toIndex] + new Vector2(-tangents[toIndex].y, tangents[toIndex].x)) +
        //h3 * tangents[fromIndex] +
        //h4 * tangents[toIndex];
        //sides[1] = h1 * (points[fromIndex] - new Vector2(-tangents[fromIndex].y, tangents[fromIndex].x)) +
        //h2 * (points[toIndex] - new Vector2(-tangents[toIndex].y, tangents[toIndex].x)) +
        //h3 * tangents[fromIndex] +
        //h4 * tangents[toIndex];        

        Vector2[] fromSidePoints = GetSidePoints(fromIndex);
        Vector2[] toSidePoints = GetSidePoints(toIndex);
        
        sides[0] = h1 * (fromSidePoints[0]) +
        h2 * (toSidePoints[0]) +
        h3 * tangents[fromIndex] +
        h4 * tangents[toIndex];
        sides[1] = h1 * (fromSidePoints[1]) +
        h2 * (toSidePoints[1]) +
        h3 * tangents[fromIndex] +
        h4 * tangents[toIndex];
